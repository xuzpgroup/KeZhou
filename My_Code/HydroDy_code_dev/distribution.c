    ...
    double *box, char *filename)
{
    ...
    while (feof(fin)==0)
    {
        ...
        // if (len % 1==0) printf("Now is %d\n", len);
        fscanf(fin, "%d\n", &natoms);
        fgets(header_char, 300, fin);
        // puts(header_char);
        // printf("len=%d -- Natoms = %d \n", natoms);
        for (i=0;i<natoms;i++)
        {
            ...
            // printf("%d - %d %c tp=%d, z=%lf, mass=%lf\n",\
             len, i+1, char_tp, tp[i], r[i][2], mass[i]);      
        }
    ...
    double *box, char *filename)
{
   int    DIM;
    ...
    total_len=0;
    fin=fopen(filename, "r");
    fH=fopen("proton_sequence.dat", "w");  
    fprintf(fH, "####  t  #### n_H \n"); 
    sn_H=0;
    ...
    while (feof(fin)==0)
    {
        ...
        // calculate the number of proton 
        n_H=0;
        for (i=0;i<natoms;i++)
        {
            npH=0;
            ...
            for (j=0;j<natoms;j++)
            {
                if (i!=j && tp[j]==8)
                {
                    if (Distant_PBC(r[i], r[j], box)<Cut_off_OH)
                    {
                        npH++;
                    }
                } 
    ...
    printf(" The average number of proton is %lf \n", (sn_H+0.0)/total_len);
    fprintf(fH, "#### n(H+) ######\n");
    fprintf(fH, "#### %lf \n", (sn_H+0.0)/total_len);
    fclose(fH);
